...
import trimesh
# import libmcubes
from .libmcubes import marching_cubes
...
class Extractor3D(object):
    ...
    def __init__(self, input_fn, field_fn, points_batch_size=10000,
                 threshold=0, refinement_step=0, device=None,
                 resolution0=16, upsampling_steps=0,
    ...
    def generate_mesh(self, encoder_states, data=None, return_stats=True, mask_loader=None):
        ...
        mesh = self.generate_from_latent(encoder_states, None, stats_dict=stats_dict,
                                         data=None, mask_loader=mask_loader, **kwargs)

        return mesh, stats_dict

    def generate_from_latent(self, encoder_states, c=None, stats_dict={}, data=None,
        ...
        if self.upsampling_steps == 0:
            ...
            pointsf = voxel_centers.unsqueeze(1) + pointsf.unsqueeze(0)
            pointsf = pointsf.reshape(-1, 3)
            values = self.eval_points(
                pointsf, encoder_states, c, **kwargs).cpu().numpy()
            value_grid = values.reshape(-1, nx, nx, nx)
        else:
            pass
            # mesh_extractor = MISE(
        ...
        mesh = self.extract_mesh(
            value_grid, voxel_centers, c, stats_dict=stats_dict)
        return mesh

    def eval_points(self, p, encoder_states, c=None, **kwargs):
        ...
        for pi in p_split:
            with torch.no_grad():
                sample_embeddings, _ = self.input_fn.get_point_embedding(
                    pi, encoder_states)
                # print(pi[:20])
                # print(sample_embeddings[:20, -3:])
                occ_hat = -1.0 * self.field_fn(sample_embeddings).squeeze(-1)
                # print(occ_hat[:20])
    ...
    def extract_mesh(self, sdf_values, voxel_centers, c=None, stats_dict=dict()):
        ''' Extracts the mesh from the predicted occupancy grid.

        Args:
            sdf_values (tensor): value grid of sdf
            c (tensor): latent conditioned code c
            stats_dict (dict): stats dictionary
        '''
        # Some short hands
        n_v, n_x, n_y, n_z = sdf_values.shape
        threshold = 0
        # threshold = np.log(self.threshold) - np.log(1. - self.threshold)
        vertices_list = []
        triangles_list = []
        ...
        for v in range(n_v):
            ...
            sdf_grid = sdf_values[v]
            # t0 = time.time()
            # occ_hat_padded = np.pad(
            #     occ, 1, 'constant', constant_values=-1e6)
            vertices, triangles = marching_cubes(sdf_grid, threshold)
            # stats_dict['time (marhing cubes)'] = time.time() - t0
            ...
            vertices = 2 * self.padding * vertices - \
                np.full_like(vertices, self.padding)
            vertices = center.unsqueeze(0).cpu().numpy() + vertices
    ...
    def estimate_normals(self, vertices, c=None):
        ...
        for vi in vertices_split:
            ...
            vi.requires_grad_()
            occ_hat = self.model.decoder(
                vi, None, only_occupancy=True).squeeze(-1)
            out = occ_hat.sum()
            out.backward()
            ni = -vi.grad
        ...
        while it_r < self.refinement_step:
            for f_it in dataloader:
                ...
                face_value = torch.cat([
                    ...
                    self.model.decoder(
                        p_split, None, only_occupancy=True).squeeze(-1)
