...
class SparseVoxelOctreeRayIntersect(Function):
    ...
    def forward(ctx, voxelsize, n_max, points, children, ray_start, ray_dir):
        G = min(1024, int(2 * 10 ** 9 / (points.numel() + children.numel())))   # HACK: avoid out-of-memory
        N = ray_start.shape[0]
        K = int(np.ceil(N / G))
        H = K * G
        # print(ray_start.shape)
        if H > N: # duplicate for batch alignment
            # ray_start = torch.cat([ray_start, ray_start[:H-N]], 0)
            # ray_dir = torch.cat([ray_dir, ray_dir[:H-N]], 0)
            
            ray_start = torch.cat([ray_start, ray_start[:1].expand(H-N, -1)], 0)
            ray_dir = torch.cat([ray_dir, ray_dir[:1].expand(H-N, -1)], 0)
            
        # print(ray_start.shape, G, K)
        ray_start = ray_start.reshape(G, K, 3)
        ray_dir = ray_dir.reshape(G, K, 3)
        points = points.expand(G, *points.shape).contiguous()
        children = children.expand(G, *children.shape).contiguous()
        
        max_hit = n_max * 10
        inds, min_depth, max_depth = _ext.svo_intersect(
            ray_start.float(), ray_dir.float(), points.float(), children.int(), voxelsize, max_hit)
        ...
        idx = torch.argsort(torch.where(min_depth < 1e-7, torch.full_like(min_depth, 1e10), min_depth), dim=-1)
        
        n_max = n_max.int()
        inds = torch.gather(inds, -1, idx)[..., :n_max]
        min_depth = torch.gather(min_depth, -1, idx)[..., :n_max]
        max_depth = torch.gather(max_depth, -1, idx)[..., :n_max]
...
class InverseCDFRaySampling(Function):
    @staticmethod
    def forward(ctx, pts_idx, min_depth, max_depth, probs, steps, fixed_step_size=-1):
        ...
        noise = min_depth.new_zeros(*min_depth.shape[:-1], max_steps)
        noise = noise.uniform_().clamp(min=0.001, max=0.999)  # in case
        
        # print(min_depth.shape, noise.shape, max_steps)
        ...
        sampled_idx, sampled_min_depth, sampled_max_depth, sampled_depth, sampled_dists = [
            torch.cat([r[i] for r in results], 1)
            for i in range(5)
        ]
        
        sampled_min_depth = sampled_min_depth.type_as(min_depth)
        sampled_max_depth = sampled_max_depth.type_as(min_depth)
        sampled_depth = sampled_depth.type_as(min_depth)
        sampled_dists = sampled_dists.type_as(min_depth)
        
        sampled_idx = sampled_idx.reshape(H, -1)
        sampled_min_depth = sampled_min_depth.reshape(H, -1)
        sampled_max_depth = sampled_max_depth.reshape(H, -1)
        sampled_depth = sampled_depth.reshape(H, -1)
        sampled_dists = sampled_dists.reshape(H, -1)

        if H > N:
            sampled_idx = sampled_idx[: N]
            sampled_min_depth = sampled_min_depth[: N]
            sampled_max_depth = sampled_max_depth[: N]
            sampled_depth = sampled_depth[: N]
            sampled_dists = sampled_dists[: N]

        max_len = sampled_idx.ne(-1).sum(-1).max()
        sampled_idx = sampled_idx[:, :max_len]
        sampled_min_depth = sampled_min_depth[:, :max_len]
        sampled_max_depth = sampled_max_depth[:, :max_len]
        sampled_depth = sampled_depth[:, :max_len]
        sampled_dists = sampled_dists[:, :max_len]
 
        ctx.mark_non_differentiable(sampled_idx)
        ctx.mark_non_differentiable(sampled_min_depth)
        ctx.mark_non_differentiable(sampled_max_depth)
        ctx.mark_non_differentiable(sampled_depth)
        ctx.mark_non_differentiable(sampled_dists)

        return sampled_idx, sampled_min_depth, sampled_max_depth, sampled_depth, sampled_dists
